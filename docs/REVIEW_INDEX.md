...
> 状态：**设计及扩展均已审查通过，0.3.2 实现、测试、相似性复核与真实 API 验收已完成**
> 调研日期：2026-08-28  
...
8. [0.3.1 真实 API 验收报告](08_REAL_API_ACCEPTANCE_REPORT.md)
...
用户配置 `.env` 后又完成三个真实 API 隔离案例。核心创建、修复、复验、Git 观察和凭证提交
均通过；验收同时发现 `run_local` 在自动批准模式下可以修改用户级配置。测试产生的精确 Git
配置已撤销，风险、证据与后续加固建议记录在第 8 份报告中。

`0.3.2` 根据长任务真实运行日志修复空 assistant 上下文回放：模型偶发返回既无文本也无工具
调用的消息时，RunBook 仍保留原始事实，但下一次 API 请求只回放控制提示，不再向兼容网关
发送非法空消息。合法的 `content: null + tool_calls` 不受影响。
